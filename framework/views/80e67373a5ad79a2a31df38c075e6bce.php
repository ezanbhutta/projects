<footer>
    <div class="tp-footer-area tp-footer-style-2 tp-footer-style-primary tp-footer-style-6" data-bg-color="#F4F7F9">
        <div class="tp-footer-top pt-95 pb-40">
            <div class="container">
                <div class="row">
                    <div class="col-xl-4 col-lg-3 col-md-4 col-sm-6">
                        <div class="tp-footer-widget footer-col-1 mb-50">
                            <div class="tp-footer-widget-content">
                                <div class="tp-footer-logo ">
                                    <a href="<?php echo e(url('/')); ?>">
                                        <img src="<?php echo e(asset('logo 4.png')); ?>" width="100px" height="70px" alt="logo">
                                    </a>
                                </div>
                                <p class="tp-footer-desc">The Chaandi Kaar e-commerce platform addresses the challenges
                                    of traditional silver jewelry shopping in Multan</p>
                                <div class="tp-footer-social">
                                    <a href="https://www.facebook.com/"><i class="fa-brands fa-facebook-f"></i></a>
                                    <a href="https://www.twitter.com/"><i class="fa-brands fa-twitter"></i></a>
                                    <a href="https://www.linkedin.com/"><i class="fa-brands fa-linkedin-in"></i></a>
                                    <a href="https://www.vimeo.com/"><i class="fa-brands fa-vimeo-v"></i></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
                        <div class="tp-footer-widget footer-col-2 mb-50">
                            <h4 class="tp-footer-widget-title">My Account</h4>
                            <div class="tp-footer-widget-content">
                                <ul>
                                    <li><a href="<?php echo e(url('/account')); ?>">My Account</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
                        <div class="tp-footer-widget footer-col-3 mb-50">
                            <h4 class="tp-footer-widget-title">Infomation</h4>
                            <div class="tp-footer-widget-content">
                                <ul>
                                    <li><a href="<?php echo e(url('/about')); ?>">About Us</a></li>
                                    <li><a href="<?php echo e(url('/shop')); ?>">Shop</a></li>
                                    <li><a href="<?php echo e(url('/contact')); ?>">Contact Us</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
                        <div class="tp-footer-widget footer-col-4 mb-50">
                            <h4 class="tp-footer-widget-title">Talk To Us</h4>
                            <div class="tp-footer-widget-content">
                                <div class="tp-footer-talk mb-20">
                                    <span>Got Questions? Contact Us</span>
                                </div>
                                <div class="tp-footer-contact">
                                    <div class="tp-footer-contact-item d-flex align-items-start">
                                        <div class="tp-footer-contact-icon">
                                            <span>
                                                <svg width="18" height="16" viewBox="0 0 18 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M1 5C1 2.2 2.6 1 5 1H13C15.4 1 17 2.2 17 5V10.6C17 13.4 15.4 14.6 13 14.6H5"
                                                        stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                    <path
                                                        d="M13 5.40039L10.496 7.40039C9.672 8.05639 8.32 8.05639 7.496 7.40039L5 5.40039"
                                                        stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M1 11.4004H5.8" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path d="M1 8.19922H3.4" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                </svg>
                                            </span>
                                        </div>
                                        <div class="tp-footer-contact-content">
                                            <p><a href="mailto:rishalsiddiqui87@gmail.com"><span class="__cf_email__"
                                                        data-cfemail="196a71767f60596a6c6969766b6d377a7674">rishalsiddiqui87@gmail.com</span></a>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="tp-footer-contact-item d-flex align-items-start">
                                        <div class="tp-footer-contact-icon">
                                            <span>
                                                <svg width="18" height="16" viewBox="0 0 18 16" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M1 5C1 2.2 2.6 1 5 1H13C15.4 1 17 2.2 17 5V10.6C17 13.4 15.4 14.6 13 14.6H5"
                                                        stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                    <path
                                                        d="M13 5.40039L10.496 7.40039C9.672 8.05639 8.32 8.05639 7.496 7.40039L5 5.40039"
                                                        stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M1 11.4004H5.8" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path d="M1 8.19922H3.4" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                </svg>
                                            </span>
                                        </div>
                                        <div class="tp-footer-contact-content">
                                            <p><a href="mailto:ezanmujahid@gmail.com"><span class="__cf_email__"
                                                        data-cfemail="196a71767f60596a6c6969766b6d377a7674">ezanmujahid@gmail.com</span></a>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="tp-footer-contact-item d-flex align-items-start">
                                        <div class="tp-footer-contact-icon">
                                            <span>
                                                <svg width="18" height="16" viewBox="0 0 18 16"
                                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M1 5C1 2.2 2.6 1 5 1H13C15.4 1 17 2.2 17 5V10.6C17 13.4 15.4 14.6 13 14.6H5"
                                                        stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path
                                                        d="M13 5.40039L10.496 7.40039C9.672 8.05639 8.32 8.05639 7.496 7.40039L5 5.40039"
                                                        stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path d="M1 11.4004H5.8" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                    <path d="M1 8.19922H3.4" stroke="currentColor" stroke-width="1.5"
                                                        stroke-miterlimit="10" stroke-linecap="round"
                                                        stroke-linejoin="round" />
                                                </svg>
                                            </span>
                                        </div>
                                        <div class="tp-footer-contact-content">
                                            <p><a href="mailto:chusamadon31@gmail.com"><span class="__cf_email__"
                                                        data-cfemail="196a71767f60596a6c6969766b6d377a7674">chusamadon31@gmail.com</span></a>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="tp-footer-contact-item d-flex align-items-start">
                                        <div class="tp-footer-contact-icon">
                                            <span>
                                                <svg width="17" height="20" viewBox="0 0 17 20"
                                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M8.50001 10.9417C9.99877 10.9417 11.2138 9.72668 11.2138 8.22791C11.2138 6.72915 9.99877 5.51416 8.50001 5.51416C7.00124 5.51416 5.78625 6.72915 5.78625 8.22791C5.78625 9.72668 7.00124 10.9417 8.50001 10.9417Z"
                                                        stroke="currentColor" stroke-width="1.5" />
                                                    <path
                                                        d="M1.21115 6.64496C2.92464 -0.887449 14.0841 -0.878751 15.7889 6.65366C16.7891 11.0722 14.0406 14.8123 11.6313 17.126C9.88298 18.8134 7.11704 18.8134 5.36006 17.126C2.95943 14.8123 0.210885 11.0635 1.21115 6.64496Z"
                                                        stroke="currentColor" stroke-width="1.5" />
                                                </svg>
                                            </span>
                                        </div>
                                        <div class="tp-footer-contact-content">
                                            <p><a href="https://maps.app.goo.gl/gpaAD5fUgrSsPsSYA" target="_blank">The
                                                    Chaandi Kaar e-commerce <br>Multan </a></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="tp-footer-bottom">
            <div class="container">
                <div class="tp-footer-bottom-wrapper">
                    <div class="row align-items-center">
                        <div class="col-md-6 mx-auto">
                            <div class="tp-footer-copyright">
                                <p>© <?php echo e(date('Y')); ?> All Rights Reserved | Chaandi Kaar E-commerce by <a
                                        href="https://ibexstack.com/">ibexstack</a>.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<script data-cfasync="false" src="<?php echo e(asset('/../../cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js')); ?>">
</script>
<script src="<?php echo e(asset('/user/assets/js/vendor/jquery.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/vendor/waypoints.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/bootstrap-bundle.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/meanmenu.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/swiper-bundle.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/slick.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/range-slider.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/magnific-popup.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/nice-select.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/purecounter.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/countdown.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/wow.js')); ?>"></script>
<script src="<?php echo e(asset('admin/assets/js/sweat@alert.js')); ?>"></script>
<script src="<?php echo e(asset('admin/assets/js/delete_data.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/isotope-pkgd.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/imagesloaded-pkgd.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/ajax-form.js')); ?>"></script>
<script src="<?php echo e(asset('/user/assets/js/main.js')); ?>"></script>
<?php echo $__env->yieldContent('script'); ?>
<?php /**PATH C:\xampp\htdocs\channdi\resources\views/components/user-footer.blade.php ENDPATH**/ ?>